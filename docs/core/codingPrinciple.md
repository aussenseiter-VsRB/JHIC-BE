...
Split repository interfaces by concern. The concrete `repository_pg.go` implements all of them, but consumers only depend on what they need.

### 3. Colocation over Centralization

Code lives next to what it serves:
- Domain entity lives in `internal/domain/{name}/entity.go`
- Repository interface and implementation live in the same package
...
| Postgres implementation | `repository_pg.go` | `type RepositoryPG struct` |
