...
5. **Domain structure.** Each business domain is a top-level package under `internal/domain/`. Every domain has: `entity.go`, `repository.go`, `service.go`, `handler.go`, plus a `pg/` subpackage for the Postgres implementation and its integration tests. Optionally `types.go` or `errors.go` for auxiliary types.
...
5. Write your code following the documented patterns.
6. If modifying existing docs, only touch `type: editable` files.
